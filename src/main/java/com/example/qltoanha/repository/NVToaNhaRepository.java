...
public interface NVToaNhaRepository extends CrudRepository<NhanVienToaNha,Integer> {
    ...
    @Query(value = "select * from tbl_nv_toa_nha where tbl_nv_toa_nha.ma_phong_ban in (select id from tbl_phong_ban where tbl_phong_ban.toa_nha_id = ?2 and id = ?1) " , nativeQuery = true)
	List<NhanVienToaNha>findByPhongBan(int id,int toanha);
